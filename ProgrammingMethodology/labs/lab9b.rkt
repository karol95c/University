...
(define evaluator
  '(let (bin-op (lambda (x) (car x)))
     (let (left-expr (lambda (x) (car (cdr x))))
       (let (right-expr (lambda (x) (car (cdr (cdr x)))))
         (lambda-rec (eval-interp e)
                     (if (pair? e)
                         (cond
                           [(eq? (bin-op e) '+)
                            (+ (eval-interp (left-expr e)) (eval-interp (right-expr e)))]
                           [(eq? (bin-op e) '-)
                            (- (eval-interp (left-expr e)) (eval-interp (right-expr e)))]
                           [(eq? (bin-op e) '*)
                            (* (eval-interp (left-expr e)) (eval-interp (right-expr e)))]
...
(eval (list evaluator '(cons '*
                             (cons  (cons '/ (cons 2 (cons 3 null)))
                                    (cons (cons '* (cons 2 (cons 3 null)))
                                          null)))))
(eval (list evaluator '(cons '+ (cons 1 (cons 2 null)))))
;(1 + (1 + 2) = 3
(eval (list evaluator '(cons '+ (cons 1 (cons (cons '+ (cons 1 (cons 2 null))) null)))))
;blad dzielenia przez zero
;(eval (list evaluator '(cons '/ (cons 1 (cons 0 null)))))
;(10 - (20 + 30)) = - 40
(eval (list evaluator '(cons '- (cons 10 (cons (cons '+ (cons 20 (cons 30 null))) null)))))
;((2 * 8) + (3 + 5)) / ((3 + 7) - (25 / 2)) = 24 / -2.5 = - 9.6
(eval (list evaluator '(cons '/ (cons (cons '+
                                         (cons (cons '* (cons 2 (cons 8 null)))
                                               (cons (cons '+ (cons 3 (cons 5 null)))
                                                     null)))
                                   (cons (cons '-
                                               (cons (cons '+ (cons 3 (cons 7 null)))
                                                     (cons (cons '/ (cons 25 (cons 2 null)))
                                                           null))) null)))))

;((1/8)*(1/2))=1/16
(eval (list evaluator '(cons '* (cons  (cons '/ (cons 1 (cons 8 null)))
