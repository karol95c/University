...
(define (is-empty? xs ys)
  (or (null? xs) (null? ys)))

(define (find-common-var xs ys)
  (if (is-empty? xs ys)
      false
      (let ((x (car xs))
            (y (car ys)))
        (cond [(var=? x y) x]
              [(var<? x y) (find-common-var (cdr xs) ys)]
              [else (find-common-var xs (cdr ys))]))))

(define (clause-trivial? c)
  (if (not (find-common-var (res-clause-pos c) (res-clause-neg c)))
      false
      true))

(define (delete-var v xs)
  (cond [(null? xs) '()]
        [(equal? v (car xs)) (cdr xs)]
        [else (cons (car xs) (delete-var v (cdr xs)))]))

(define (merge-and-del-duplicates xs ys)
  (remove-duplicates-vars (merge-vars xs ys)))

(define (resolve c1 c2)
  (let* ((pos-c1 (res-clause-pos c1))
         (neg-c1 (res-clause-neg c1))
         (pos-c2 (res-clause-pos c2))
         (neg-c2 (res-clause-neg c2))
         (x (find-common-var pos-c1 neg-c2)))
    (if (var? x)
        (let ((pos (merge-and-del-duplicates (delete-var x pos-c1) pos-c2))
              (neg (merge-and-del-duplicates neg-c1 (delete-var x neg-c2))))
          (res-clause pos neg
                      (proof-res x (res-clause-proof c1) (res-clause-proof c2))))
        (let ((y (find-common-var neg-c1 pos-c2)))
          (if (var? y)
              (let ((pos (merge-and-del-duplicates pos-c1 (delete-var x pos-c2)))
                    (neg (merge-and-del-duplicates (delete-var x neg-c1) neg-c2)))
                (res-clause pos neg
                            (proof-res x (res-clause-proof c1) (res-clause-proof c2))))
              false)))))

      
  ;    ((define (make-clause xs ys hx hy)
   ;      (let ((v1 (car (res-clause-pos xs)))
    ;           (v2 (car (res-clause-neg ys)))          
     ;    (cond [(var=? v1 v2) 
...
(define clause1 (clause (literal #t 'p) (literal #t 'q)
                        (literal #t 'r)))
(define clause2 (clause (literal #f 'q) (literal #f 's)
                        (literal #t 't) (literal #f 'u)))
(define clause-trivial1 (clause (literal #t 'p) (literal #t 'q)
                                (literal #t 'r) (literal #f 'q)))
(define cnf1 (cnf clause1 clause2))
(define cnf-trivial (cnf clause-trivial1))
(define c1 (form->clauses cnf1))
(define c-trivial (form->clauses cnf-trivial))

(clause-trivial? (car c-trivial))
(clause-trivial? (car c1))
(clause-trivial? (cadr c1))
(resolve (car c1) (cadr c1))

(define clause3 (clause (literal #f 'r) (literal #t 's)
                        (literal #t 'u)))
(define clause4 (clause (literal #f 'p) (literal #t 'q)
                        (literal #f 't) (literal #f 'w)))
(define cnf2 (cnf clause3 clause4))

(define c-no-resolvents (form->clauses cnf2))
(resolve (car c-no-resolvents) (cadr c-no-resolvents))
