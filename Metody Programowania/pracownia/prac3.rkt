...
(define (solve-ax+by=1 a b)
  (define (switch res)
    (let ([x (car res)]
         ...
         (cons y x)))  
   (if (> b a)
       (switch (solve-ax+by=1 b a))
       (let ([q (floor (/ a b))]
            [r (- a (* (floor (/ a b)) b))])
            (if (= r 1)
                (cons 1 (* q -1))
                (let ([p (solve-ax+by=1 b r)])
    ...
    (define (unconvert l sum)
        (if (null? l)
          ...
          (let ((x (modulo (+ (RSA-transform (car l)
                                  key) sum) n)))
            (cons x (unconvert (cdr l) (modulo (car l ) n))))))
    (unconvert intlist 0)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ([mess (RSA-encrypt message recipient-public-key)])
        (let ([sign (RSA-convert-list (cons (compress mess) null) sender-private-key)])
          (make-signed-message sign mess))))
        
(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ([mess (RSA-decrypt (get-message cyphertext) recipient-private-key)])
        (let ([sign (RSA-convert-list (get-signature cyphertext) sender-public-key)]
              [expected (compress (get-message cyphertext))])
          (if (= expected (car sign))
              mess
