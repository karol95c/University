...
;kiedy ewaluator napotka lazy-let
;dodaje on do srodowiska zmienna ktorej wartoscia jest domkniecie
;w przypadku proby znalezienia zmiennej w srodowisku funkcja find-in-enviroment
;obliczy wartosc domkniecia obliczajac eval-env dla wyrazenia domkniecia i jego srodowiska
;zmienne domkniecia mozemy pominac, poniewaz chcemy funkcje naszego domkniecia
;traktowac jako bezargumentowa
        ...
        [(lazy-let? e)
         (eval-env (let-expr e)
                   (add-to-env (lazy-let-def-var (lazy-let-def e))
                               (closure-cons '()
                                  (lazy-let-def-expr (lazy-let-def e)) env)
                               env))]
        [(my-null? e)
         null]
        [(cons? e)
...
;lekko zmodyfikowane wyszkiwanie zwracanie zmiennej ze srodowiska 
(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(eq? x (caar env))
         (let ((c (cadar env)))
           (if (closure? c)
               (eval-env (closure-expr c) (closure-env c))
               c))]
        [else (find-in-env x (cdr env))]))

;lazy-let (budowa taka jak w zwyklym let)
...
;proste testy
;blad w definicji leta, ale wartosc nie zostaje wywolana do obliczenia
(eval '(lazy-let (x (/ 5 0)) 5))
;definicja bez bledu i wywolanie
(eval '(lazy-let (x (/ 5 1)) (+ x 10)))
;zwykly let i lazy-let
(eval '(let (x 10) (lazy-let (y 5) (+ x y))))
;przesloniecie x letem i wywolanie wartosci x
(eval '(lazy-let (x (/ 5 0)) (let (x 10) (+ x 2))))
;przesloniecie x kolejnym lazy-letem i wywolanie wartosci x 
(eval '(lazy-let (x (/ 5 0)) (lazy-let (x 2) (+ x 2))))
;blad w definicji leta
(eval '(lazy-let (x (car 1)) 1))
;blad wywolanie x, ktore oblicza (/ 1 0), zwraca blad: division by zero
(eval '(lazy-let (x (/ 1 0)) (+ 1 x)))
